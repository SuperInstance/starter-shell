"""flux-lsp headspace — Language Server Protocol for FLUX ISA.

This headspace wraps the flux-lsp TypeScript LSP server from the
SuperInstance/flux-lsp repo. It requires Node.js >=18 and npm packages
from the flux-lsp package.json.
...
VSCode Integration:
    ...
    Or use the LSP client wrapper in client/extension.ts from the flux-lsp repo.

Environment Variables:
    FLUX_LSP_PORT   — port for LSP server (default: 5100)
    FLUX_LSP_PATH   — path to flux-lsp repo (default: /tmp/flux-lsp)
"""
...
def _build_if_needed():
    ...
    if os.path.exists(dist):
        return True
    print(f"🔧 flux-lsp: building TypeScript server (first time setup)...")
    try:
        r = subprocess.run(
        ...
        if r2.returncode != 0:
            ...
            return False
        print("✅  flux-lsp: build complete")
        return True
    except Exception as e:
        print(f"⚠️  flux-lsp: build error {e}")
        return False
...
def _run_server():
    ...
    if not os.path.exists(server_js):
        print(f"❌ flux-lsp: server.js not found at {server_js}")
        return

    RUNNING = True
    print(f"🚀 flux-lsp: starting LSP server on port {LSP_PORT}...")
    try:
        ...
        print(f"✅ flux-lsp: LSP server running (pid {PROCESS.pid})")
        PROCESS.wait()
    except Exception as e:
        print(f"❌ flux-lsp: server error: {e}")
    finally:
        RUNNING = False
...
def stop():
    global PROCESS, RUNNING
    if PROCESS:
        print(f"🛑 flux-lsp: stopping LSP server (pid {PROCESS.pid})...")
        PROCESS.terminate()
        try:
